## 9-7 신규 아이디 추천 (프로그래머스 2021 KAKAO BLIND RECRUITMENT)
...
문제의 알고리즘 자체는 간단하다.  순서대로 해주면 된다.

정규식을 잘 써서 특수문자를 없애는 스킬이 필요한 것 같다.
...
class Solution {
    public String solution(String new_id) {
        ...
        new_id = new_id.replaceAll(filter, "");
        ...
        while(new_id.length() <= 2) {
        	new_id = new_id.concat(new_id.charAt(new_id.length()-1)+"");
        }
        return new_id;
    }
}
